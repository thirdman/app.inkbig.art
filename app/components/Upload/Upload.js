...
//    const {} = this.props;
		...
		return (
			<div className={`${styles.Upload} ${styles.wrap} ${isUpdated ? styles.isUpdated : ''}`}>
					...
					<div className={`${styles.column} `}>
						...
						{!isLoading && !files &&
							...
							{!isLoading && files && files.map((file) => {
								console.log(file);
								return (
									<section className={styles.previewSection} key={`file_${file.name}`}>
	...
	onDrop = (files) => { // eslint-disable-line
		...
		const slug = files && files[0].name && files[0].name.substr(0, files[0].name.lastIndexOf('.')).replace(/ /g, '');
		this.setState({
      files,
      filename,
      slug,
		...
		.then((snapshot) => {
			...
			.then((snap) => {
				...
				if (file) {
					...
					.then((svgRef) => {
						console.log('SVG successfully added to db, with reference of: ', svgRef);
						this.setState({
							isError: false
						});
					})
